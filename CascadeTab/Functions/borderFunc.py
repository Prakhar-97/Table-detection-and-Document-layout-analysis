...
def extract_table(table_body,__line__,lines=None):
    ...
    # Check if table image is  bordered or borderless
        ...
        print ("temp_lines_hor",temp_lines_hor)
        print ("temp_lines_ver",temp_lines_ver)
    else:
        temp_lines_hor, temp_lines_ver = lines

    if (temp_lines_hor is None) or (temp_lines_ver is None):
    ...
    filename = "cells.jpg"
    cv2.imwrite(filename, table_body)'''
    # cv2_imshow("cells",table_body)
    # cv2.waitKey(0)
    ############################
    return box
